...
export const AuthProvider = ({ children }) => {
        ...
        const checkActiveShift = async () => {
            if (user && user.role === 'Operator' && !activeShift) {
                ...
                try {
                    ...
                    await axios.post('http://localhost:5027/api/shifts/check-auto-close');
    ...
    useEffect(() => {
        const checkAutoClose = async () => {
            ...
            try {
                const response = await axios.post('http://localhost:5027/api/shifts/check-auto-close');
    ...
    const login = async (username, password) => {
        try {
            const response = await axios.post('http://localhost:5027/api/auth/login', { username, password });
    ...
    const selectEmployee = async (employeeId, employeeName) => {
        try {
            // Try to start shift
            const response = await axios.post('http://localhost:5027/api/shifts/start', parseInt(employeeId), {
            ...
            if (error.response && error.response.status === 400) {
                try {
                    const activeResponse = await axios.get(`http://localhost:5027/api/shifts/active/${employeeId}`);
                    const employeeData = { id: employeeId, name: employeeName };
                    setEmployee(employeeData);
    ...
    const endShift = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:5027/api/shifts/end', parseInt(employee.id), {
                headers: { 'Content-Type': 'application/json' }
            });
